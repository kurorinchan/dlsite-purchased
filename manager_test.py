...
class ManagerTest(unittest.TestCase):
    ...
    def testRemoveFilesInDir(self):
        with TemporaryDirectory() as tmpdir:
            ...
            (dir_with_items / 'a subdirectory 32001230178').mkdir()
    ...
    def testExtract(self, mock_unarchive, mock_create_archive_dirs, mock_move):
        with TemporaryDirectory() as tmpdir:
            ...
            mock_move.assert_any_call(Path(download_dir / 'RJ123'),
                                      extracted_files_dir)

            mock_move.assert_any_call(Path(download_dir / 'RJ2786'),
                                      extracted_files_dir)
    ...
    @patch('dlsite_extract.CreateArchivesDirs')
    @patch('dlsite_extract.Unarchive')
    def testExtractAlreadyExtracted(self, mock_unarchive,
                                    mock_create_archive_dirs, mock_move,
                                    mock_rmtree):
        with TemporaryDirectory() as tmpdir:
            download_dir = Path(tmpdir) / 'downloads'
            download_dir.mkdir()
            ...
            mock_move.assert_called_once_with(Path(download_dir / 'RJ123'),
                                              extracted_files_dir)
